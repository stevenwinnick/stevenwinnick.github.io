...
export const PROJECTS: Project[] = [
  {
    ...
    description: (
      <>
        <p>{`In my last semester of college, I decided to take an Electrical Engineering capstone class, `}<ProseLink href="http://www.cs.columbia.edu/~sedwards/classes/2024/4840-spring/index.html" external>Embedded Systems</ProseLink>{`, despite not actually being an Electrical Engineering major. I had really enjoyed an EE class that I'd taken in which we'd built up a working CPU able to execute assembled machine code from basic logic gates, and I was curious about how other hardware systems worked. I had also taken Operating Systems the semester before and was curious about the physical connections between hardware peripherals and software. I hadn't taken the recommended prerequisite classes for Embedded Systems, but I was curious enough about its content that I figured I'd be able to catch up where I needed. The class ended up being a great experience. I learned all about the most important components of modern embedded systems (memory, networking, USB, video, etc.), including their histories and how they're commonly implemented now.`}</p>
        <p>{`The course also included a semester-long group project in which we got to work closely with our professor to design and then implement a project with both hardware and software components on an FPGA. My group of 3 decided to build a pitch shifter/scaler that would take in audio from our FPGA's 3.5mm audio input line and scale it's pitch entirely in hardware using a custom circuit we designed, then play back the audio to the board's output line, with the level of pitch scaling being controlled via software. To do so, we first started by researching different pitch scaling algorithms and implementing them in Python to gain a thorough understanding of the workings of each and decide which sounded the best for our use case. We landed on a version of the `}<ProseLink href="https://en.wikipedia.org/wiki/Phase_vocoder" external>Phase Vocoder</ProseLink>{` algorithm with transient detection for improved vertical coherence. We then implemented the algorithm in C, simulating intended hardware implementation by using fixed-size memory buffers, polar coordinate conversion of transformed frequency-domain signals to remove the need for trigonometric hardware calculations, and a real-time streaming interface, among others features. Once we were satisfied with the sound of our C implementation, we got to work designing circuits for our board, which was able to reconfigure itself to test our specified circuitry. You can find the code for Pitch Perfect `}<ProseLink href="https://github.com/sanjayrajasekharan/pitch-perfect" external>here</ProseLink>{` and our final presentation slides `}<ProseLink href="http://www.cs.columbia.edu/~sedwards/classes/2024/4840-spring/reports/Harmony-Hand-presentation.pdf" external>here</ProseLink>{`.`}</p>
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    description: (
      <p>{`As part of my music analysis work, I had the idea to analyze song lyrics through the lens of word sounds, as opposed to something like lyrical content. When starting this project, I realized that no dataset existed parsing lyrics by sound and decided to create one for myself. I also thought that a network connecting words by their shared sounds would be useful both for this project and potentially other related ones, which ended up becoming RhymeNet. To create this database, I started with the `}<ProseLink href="http://www.speech.cs.cmu.edu/cgi-bin/cmudict" external>CMU Pronouncing Dictionary</ProseLink>{`, which contains the sounds/phonemes used to pronounce over 134,000 words. Because syllable divisions are so important in song lyrics, I created an algorithm to automatically parse these phoenetic pronunciations of words into syllables based on the linguistic theory of `}<ProseLink href="https://www.tandfonline.com/doi/abs/10.1080/00437956.1978.11435661" external>{`"greedy onset clustering"`}</ProseLink>{` and added sections of the network connecting words by their shared syllables, vowel sounds, and rhyming endings. You can download RhymeNet `}<ProseLink href="https://github.com/stevenwinnick/RhymeNet" external>here</ProseLink>{`.`}</p>
    ),
  },
  {
    id: "sheepshead",
    title: "SHEEPSHEAD AI",
    subtitle: (
      <>{`During our Spring Break, my friend `}<ProseLink href="https://www.linkedin.com/in/adam-strupp-51b82319a/" external>Adam</ProseLink>{` and I built a simulator of our favorite card game, Sheepshead, then trained an AI to play it using Monte Carlo Simulation and Deep Reinforcement Learning`}</>
    ),
  ...
  },
  {
    ...
    description: (
      <p>{`In my free time, I love to make music with my MIDI keyboard. I always found it strange, though, that my keyboard had buttons that weren't used by the music creation software it came bundled with, and that the same software had QWERTY keyboard shortcuts that weren't playable with my MIDI one. To rectify this, I found software that would let me see the exact inputs my laptop was registering when I pressed different buttons on my keyboard, then created scripts so that my computer would reinterpret those buttons as QWERTY keys before they were registered by the music software. The code will work for anyone with the same keyboard, and can be found `}<ProseLink href="https://github.com/stevenwinnick/SessionViewMIDI" external>here</ProseLink>{` along with instructions for using it.`}</p>
    ),
  },
  {
    id: "image-captioner",
  ...
  },
  {
    ...
    description: (
      <p>{`As part of Columbia's Robotics Studio, I built a walking robot. Nicknamed the "Trice-Robot," this triceratops-fossil-shaped robot started as a hand-sketched design, which `}<ProseLink href="https://www.linkedin.com/in/yuting-li-3a0244220" external>Rex Li</ProseLink>{` and I then modeled in its entirety in SolidWorks, down to every last screw and insert. After 3D-printing and assembling the robot, we cut up and soldered cables to wire the electronics and installed an onboard computer. We used Bash scripts to configure the computer to send us an email with it's IP Address upon startup, allowing us to control our robot remotely with an SSH connection. I did some `}<ProseLink href="https://en.wikipedia.org/wiki/Inverse_kinematics#Numerical_solutions_to_IK_problems" external>math</ProseLink>{` to figure out how to rotate the servo motors to give us a smooth walking path, then wrote some error-handling code to control all 8 servos simultaneously and let our robot walk.`}</p>
    ),
  },
  {
    id: "neural-network",
    ...
    description: (
      <p>{`After my tech strategy internship ended, I wanted to get a deeper understanding of deep learning, specifically neural networks and how they work, so I built one from scratch using Python. My neural net uses a sigmoid activation function for an added computational challenge and manually calculates and applies partial derivatives for backpropagation. It can also scale to contain any number of hidden layers, each of any size, that it is initialized with, and accepts both batches and individual inputs. You can find the code for this project `}<ProseLink href="https://github.com/stevenwinnick/DeepLearningNeuralNetworkFromScratch" external>here</ProseLink>{`.`}</p>
    ),
  },
  {
  ...
  },
  {
    ...
    description: (
      <p>{`As a Product Manager intern at `}<ProseLink href="https://www.sonarhealth.co" external>Sonar</ProseLink>{`, I worked under the guidance of a team of medical experts from Columbia's Medical School to create specifications for some of the app's core features, which provide users with detailed information about specific aspects of their health. I then worked cross-functionally to guide design and engineering teams in building out these features and others. As a backend software engineer, I was able to build out some of the features I defined while learning about the software development process. Since Sonar's launch on the app store, it's been great to see that features of the app I helped build are now adding value to people's lives, especially as a user of the app myself. `}</p>
    ),
  },
  {
    id: "this-website",
    ...
    description: (
      <p>{`Rather than using a template to build my website, I decided to learn how to do it myself and use it as an opportunity to showcase my design ability. This site is built with `}<ProseLink href="https://getbootstrap.com/docs/5.2/getting-started/introduction/" external>Bootstrap 5</ProseLink>{` and fully responsive, so it should display nicely on all screen sizes and types (but if something is broken, I'd really appreciate it if you could let me know via the `}<ProseLink href="/contact">contact</ProseLink>{` page). I think it turned out pretty well, don't you? :)`}</p>
    ),
  },
];
